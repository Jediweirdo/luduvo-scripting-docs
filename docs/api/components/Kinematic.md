...
`Kinematic` is an exact, case-sensitive built-in component name. You can use it
...
This component can filter Query rows, but it does not expose a whole-value Query column in this build. Its separate scripting route is `Instance:SetBodyMotion(...)`.
