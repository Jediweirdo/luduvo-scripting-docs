...
`BrickColor` is an exact, case-sensitive built-in component name. You can use it
...
Queries expose `query.BrickColor[i]` as `vector` (read/write). Its separate scripting route is `Instance.Color: vector`.
