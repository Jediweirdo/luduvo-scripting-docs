...
`Scale` is an exact, case-sensitive built-in component name. You can use it
...
Queries expose `query.Scale[i]` as `vector` (read/write). Its separate scripting route is `Instance.Size: vector`.
